...
void* MusaBFCAllocator::AllocateRaw(size_t alignment, size_t num_bytes) {
  ...
  if (size_class > kMaxPoolSize / 2) {
    ...
    VLOG(2) << "BFCAllocator: Direct alloc " << num_bytes << " bytes at " << ptr;
    return ptr;
  }
  ...
  if (ptr) {
    ...
    VLOG(2) << "BFCAllocator: Reused " << size_class << " bytes from pool at " << ptr;
    return ptr;
  }
  ...
  if (err != musaSuccess) {
    LOG(ERROR) << "MUSA BFC Allocator: musaMalloc failed for " 
               << size_class << " bytes: " << musaGetErrorString(err);
    return nullptr;
  }
...
void MusaBFCAllocator::DeallocateRaw(void* ptr) {
  ...
  
  {
    ...
    if (it != allocated_sizes_.end()) {
      ...
      auto raw_it = std::find(musa_allocations_.begin(), musa_allocations_.end(), ptr);
      if (raw_it != musa_allocations_.end()) {
        ...
        LOG(WARNING) << "MUSA BFC Allocator: Attempting to free untracked pointer " << ptr;
        return;
      }
    }
  }  // Mutex released here
  ...
  if (is_tracked) {
    ...
    VLOG(2) << "BFCAllocator: Returned " << size_class << " bytes to pool at " << ptr;
  } else if (is_raw) {
    // Free directly
    musaSetDevice(device_id_);
    musaFree(ptr);
