...
template <typename T>
class MusaLinearLeakyReluOp : public MusaOpKernel {
 public:
  explicit MusaLinearLeakyReluOp(OpKernelConstruction* ctx)
      : MusaOpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx,
                ...
                errors::InvalidArgument(
                    "Dimension mismatch in BiasAdd of LinearLeakyRelu"));
    ...
    leaky_relu_op.SetAlpha(static_cast<double>(alpha_));
    status = leaky_relu_op.Run(handle, mt_out, mt_out);
    OP_REQUIRES(ctx, status == ::musa::dnn::Status::SUCCESS,
                errors::Internal("MUSA LeakyRelu failed in LinearLeakyRelu."));
  }
...
#define REGISTER_MUSA_LINEAR_LEAKY_RELU(TYPE)                              \
  REGISTER_KERNEL_BUILDER(Name("MusaLinearLeakyRelu")                      \
                              .Device("MUSA")                              \
                              .TypeConstraint<TYPE>("T"),                  \
                          MusaLinearLeakyReluOp<TYPE>);

REGISTER_MUSA_LINEAR_LEAKY_RELU(float);
REGISTER_MUSA_LINEAR_LEAKY_RELU(Eigen::half);
REGISTER_MUSA_LINEAR_LEAKY_RELU(bfloat16);
REGISTER_MUSA_LINEAR_LEAKY_RELU(double);

#undef REGISTER_MUSA_LINEAR_LEAKY_RELU

}  // namespace musa

REGISTER_OP("MusaLinearLeakyRelu")
    .Input("a: T")
    .Input("b: T")
    .Input("bias: T")
    .Output("product: T")
