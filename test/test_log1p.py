...
def verify_log1p(test_name, shape, dtype, tolerance=1e-4):
    ...
    try:
        with tf.device('/device:MUSA:0'):
            ...
            _ = tf.math.log1p(x_musa)
            
            start = time.time()
            res_musa = tf.math.log1p(x_musa)
...
def test_basic_cases():
    ...
    for shape, dtype, name in test_cases:
        if verify_log1p(name, shape, dtype):
            passed += 1
        total += 1
...
def test_edge_cases():
    ...
    with tf.device('/CPU:0'):
        test_values = tf.constant([-0.5, 0.0, 1.0, 2.0, 10.0], dtype=tf.float32)
        cpu_results = tf.math.log1p(test_values).numpy()
        print("CPU Log1p results:", cpu_results)
    
    try:
        with tf.device('/device:MUSA:0'):
            musa_results = tf.math.log1p(test_values).numpy()
            print("MUSA Log1p results:", musa_results)
            
            key_points = {
                "log1p(0) should be 0": musa_results[1],
                "log1p(1) should be ln(2) ¡Ö 0.693": musa_results[2],
                "log1p(2) should be ln(3) ¡Ö 1.0986": musa_results[3],
            }
            
            for desc, value in key_points.items():
                expected = {"log1p(0)": 0.0, "log1p(1)": 0.693147, "log1p(2)": 1.098612}[desc.split(" ")[0]]
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("Starting Log1p Operator Tests")
    print("="*50)
    ...
    try:
        with tf.device('/device:MUSA:0'):
            ...
            for name, values in demos:
                ...
                y = tf.math.log1p(x)
                print(f"{name}: log1p{values} = {y.numpy()}")
