...
def verify_reciprocal(test_name, shape, dtype):
    ...
    with tf.device('/CPU:0'):
        ...
        start = time.time()
        res_cpu = tf.math.reciprocal(x_cpu)
    ...
    try:
        with tf.device('/device:MUSA:0'):
            ...
            _ = tf.math.reciprocal(x_musa)
            
            start = time.time()
            res_musa = tf.math.reciprocal(x_musa)
...
def test_basic_cases():
    ...
    for shape, dtype, name in test_cases:
        if verify_reciprocal(name, shape, dtype):
            passed += 1
        total += 1
...
def test_edge_cases():
    ...
    with tf.device('/CPU:0'):
        test_values = tf.constant([0.5, 1.0, 2.0, 4.0, 10.0], dtype=tf.float32)
        cpu_results = tf.math.reciprocal(test_values).numpy()
        print("CPU Reciprocal results:", cpu_results)
    
    try:
        with tf.device('/device:MUSA:0'):
            musa_results = tf.math.reciprocal(test_values).numpy()
            print("MUSA Reciprocal results:", musa_results)
            
            key_points = {
                "reciprocal(1) should be 1": musa_results[1],
                "reciprocal(2) should be 0.5": musa_results[2],
                "reciprocal(4) should be 0.25": musa_results[3],
            }
            
            for desc, value in key_points.items():
                expected = {"reciprocal(1)": 1.0, "reciprocal(2)": 0.5, "reciprocal(4)": 0.25}[desc.split(" ")[0]]
                if abs(value - expected) < 0.01:
                    print(f"PASS {desc}: {value:.5f} ≈ {expected}")
                else:
                    print(f"FAIL {desc}: {value:.5f} ≠ {expected}")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("Starting Reciprocal Operator Tests")
    print("="*50)
    ...
    try:
        with tf.device('/device:MUSA:0'):
            demos = [
                ("Small values", [0.1, 0.5, 1.0]),
                ("Medium values", [2.0, 3.0, 4.0]),
                ("Mixed", [0.25, 1.0, 2.5]),
            ]
            ...
            for name, values in demos:
                ...
                print(f"{name}: reciprocal{values} = {y.numpy()}")
