...
本文档汇总 TensorFlow MUSA Extension 的诊断手段，包括遥测系统（Telemetry）、
图优化调试、流同步追踪以及常用环境变量组合。内存相关的诊断入口（例如
`_device_allocator_stats` / `_host_allocator_stats`）参见 `architecture-and-memory.md`。
...
内存相关环境变量（`TF_MUSA_*`）参见公共文档 `docs/environment-variables.md`。
...
1. 使用 `_device_allocator_stats` / `musaMemGetInfo` 监控显存使用趋势
   （参见 `architecture-and-memory.md`）。
...
1. 使用 Grappler / TF profiler 定位耗时热点（MUSA 插件已注册必要的 trace）
2. 验证是否启用了 TF32/AMP 加速
3. 检查 Grappler 图优化是否生效
4. 结合 `docs/environment-variables.md` 里的 H2D/D2H / 分配器参数进行调优
